...
import { PhotoIcon, PlusIcon, InformationCircleIcon, ArrowPathIcon, TrashIcon, XMarkIcon } from './Icons';
...
const CustomizeTab: React.FC<CustomizeTabProps> = ({ 
  ...
  const assetLibrary = useMemo(() => {
      ...
      // 1. Add User Custom Gallery Items (Highest Priority)
    ...
    <div className="animate-fade-in space-y-12 pb-10">
            ...
            <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[200] flex items-center justify-center p-6 animate-fade-in" onClick={() => setSelectedAsset(null)}>
                <div className="bg-bg-primary rounded-[2.5rem] shadow-2xl w-full max-w-md p-10 border border-white/10 flex flex-col" onClick={e => e.stopPropagation()}>
                    <div className="aspect-video relative rounded-2xl overflow-hidden border border-white/5">
                        ...
                        <button onClick={() => setSelectedAsset(null)} className="absolute top-4 right-4 p-2 bg-black/60 rounded-full text-white hover:bg-black/60 transition-all"><XMarkIcon className="w-5 h-5" /></button>
                    </div>
        ...
        <div className="p-5 bg-primary-accent/10 rounded-2xl border border-primary-accent/20 flex items-center gap-4">
            ...
            <p className="text-[11px] font-black uppercase tracking-widest text-primary-accent leading-relaxed">
                Visual Customization: You can upload any image. The registry automatically reshapes uploads to fit the correct dimensions (2:3 for posters, 16:9 for backdrops).
            </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <button onClick={onOpenPosterSelector} className="flex items-center gap-3 px-8 py-3.5 rounded-2xl bg-primary-accent text-on-accent font-black uppercase text-[10px] tracking-widest hover:brightness-110 transition-all shadow-2xl active:scale-95">
                <PlusIcon className="w-5 h-5" /> Add New Visual Asset
            </button>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary px-2 text-center md:text-left">Primary Poster (2:3)</h3>
                <div onClick={onOpenPosterSelector} className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5 aspect-[2/3] bg-bg-secondary/40 cursor-pointer group">
                    ...
                    <div className="absolute top-6 left-6 z-10 px-4 py-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/10 text-[9px] font-black uppercase tracking-widest text-white/80 shadow-lg">{hasCustomPoster ? 'Custom Identity Active' : 'Registry Default'}</div>
                </div>
                {hasCustomPoster && (
                    <button 
            ...
            <div className="space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-text-secondary px-2 text-center md:text-left">Primary Backdrop (16:9)</h3>
                <div onClick={onOpenBackdropSelector} className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5 aspect-video bg-bg-secondary/40 cursor-pointer group">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity"><PhotoIcon className="w-12 h-12 text-white" /></div>
                    <div className="absolute top-6 left-6 z-10 px-4 py-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/10 text-[9px] font-black uppercase tracking-widest text-white/80 shadow-lg">{hasCustomBackdrop ? 'Custom Identity Active' : 'Registry Default'}</div>
                </div>
                {hasCustomBackdrop && (
                    <button 
                    ...
                    >
                        <ArrowPathIcon className="w-4 h-4 text-primary-accent group-hover:rotate-180 transition-transform duration-500" />
                        <span>Reset to Default Backdrop</span>
                    </button>
                )}
            </div>
        </div>
        ...
        <div className="space-y-8 pt-8 border-t border-white/5">
            <div>
                ...
                <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mt-1 opacity-60">Manage all visual identifiers linked to this title. Use the trash icon to purge custom uploads.</p>
            </div>

            {assetLibrary.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-10">
                    {assetLibrary.map((asset) => (
                        <div key={asset.url} className="flex flex-col gap-3 group">
                            <div 
                                onClick={() => setSelectedAsset(asset.url)}
                                className="relative aspect-[2/3] rounded-2xl overflow-hidden border border-white/5 bg-bg-secondary/40 cursor-pointer shadow-lg hover:border-primary-accent/40 transition-all"
                            >
                                ...
                                <div className="absolute top-3 left-3">
                                    <span className={`text-[7px] font-black uppercase px-2 py-0.5 rounded-md backdrop-blur-md border border-white/10 text-white ${asset.type === 'custom' ? 'bg-primary-accent/60' : 'bg-black/60'}`}>
                                        {asset.type}
                                    </span>
                                </div>
                            </div>
                            ...
                            {asset.type === 'custom' ? (
                                <button 
                                    ...
                                    className="w-full flex items-center justify-center gap-2 py-2 bg-red-600/10 text-red-500 hover:bg-red-600 hover:text-white rounded-xl border border-red-500/20 text-[9px] font-black uppercase tracking-widest transition-all"
                                >
                                    <TrashIcon className="w-3.5 h-3.5" />
                                    Purge Asset
                                </button>
                            ) : (
                                <div className="py-2 text-center opacity-20 select-none">
                                    <span className="text-[8px] font-black uppercase tracking-widest">Locked Registry Item</span>
                                </div>
                            )}
                        </div>
                    ))}
