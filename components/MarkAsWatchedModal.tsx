...
const MarkAsWatchedModal: React.FC<MarkAsWatchedModalProps> = ({ isOpen, onClose, onSave, mediaTitle, initialScope = 'single', mediaType, showDetails, seasonDetails }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[1000] p-4" onClick={onClose}>
      <div className={`bg-bg-primary rounded-lg shadow-xl w-full p-6 animate-fade-in relative border border-white/10 flex flex-col ${showEpisodePicker ? 'max-w-2xl h-[90vh]' : 'max-w-sm'}`} onClick={e => e.stopPropagation()}>
