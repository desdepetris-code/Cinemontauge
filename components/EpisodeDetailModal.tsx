...
// Added ListBulletIcon to imports
...
const EpisodeDetailModal: React.FC<EpisodeDetailModalProps> = ({
  ...
  const stats = useMemo(() => {
    if (!episode || !showDetails || !seasonDetails) return null;
  ...
  const overview = episode.overview || "Registry data pending for this episode's plot summary.";
  ...
  const handleShare = async () => {
    const shareData = {
        title: `${showDetails.name} - ${episode.name}`,
        text: `Check out S${episode.season_number} E${episode.episode_number} of ${showDetails.name} on CineMontauge!`,
            ...
            <div className="overflow-y-auto p-10 pt-4 space-y-8 custom-scrollbar">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-[11px] font-black uppercase tracking-[0.3em] text-primary-accent">S{episode.season_number} E{episode.episode_number}</span>
                        <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                        <span className="text-[11px] font-black uppercase tracking-[0.3em] text-text-secondary">
                             {episode.runtime ? formatRuntime(episode.runtime) : 'TBA Runtime'}
                        </span>
                        <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                        <span className="text-[11px] font-black uppercase tracking-[0.3em] text-text-secondary">
                             {episode.air_date ? new Date(episode.air_date + 'T00:00:00').toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'Unscheduled'}
                        </span>
                    </div>
                    ...
                    {stats && (
                        <div className="flex flex-col gap-2 pt-2">
                            <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.2em] text-text-secondary">
                                <span>Registry Sequence Progress</span>
                                <span>{stats.watchedInSeason} / {stats.totalAired} Logged</span>
                            </div>
                ...
                {/* 3. Action Grid (Order: Watch, Rewatch, Playback, Pick, Share) */}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <button 
                        ...
                        className={`flex items-center justify-center gap-4 py-6 rounded-[2rem] font-black uppercase text-sm tracking-[0.2em] transition-all shadow-2xl active:scale-95 border ${isWeeklyPick ? 'bg-yellow-500/20 border-yellow-500 text-yellow-500' : 'bg-bg-secondary/40 border-white/5 text-text-primary'}`}
                    >
                ...
                <div className="relative">
                    <div className={showSpoilerOverlay ? 'blur-xl select-none opacity-40 grayscale' : ''}>
                        <div className="p-8 rounded-[2rem] border border-white/5 bg-bg-secondary/20 shadow-inner">
                            <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-text-secondary mb-4 opacity-40">Synopsis Registry</h3>
