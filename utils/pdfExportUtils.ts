 ...
 * Refined with comprehensive Storage RLS policies.
 */
export const generateSupabaseSpecPDF = (): void => {
    ...
    addText("CineMontauge: Supabase Backend Specification v4.0", 18, 'bold', [65, 105, 225]);
    y += 4;
    addText("PRODUCTION-GRADE RLS POLICIES & SCHEMA", 12, 'bold', [100, 100, 100]);
    addText("--------------------------------------------------------------------------------", 10, 'normal', [150, 150, 150]);
    y += 5;

    addText("1. Relational Schema Requirements", 14, 'bold');
    addText("Table: profiles -> Public metadata (Public read, Owner edit).");
    ...
    addText("Table: custom_media -> Cloud-synced asset records.");
    y += 5;

    addText("2. Secure Storage Policies (Refined)", 14, 'bold');
    addText("These rules combine Metadata, MIME-type, and Folder-path protection.", 10, 'italic');
    y += 2;
    
    addText("Bucket: avatars & custom-media", 11, 'bold');
    addText("- PUBLIC READ: TRUE (SELECT FOR ALL)");
    addText("- WRITE/UPDATE/DELETE (Role: authenticated):");
    addText("  Condition 1: bucket_id IN ('avatars', 'custom-media')");
    addText("  Condition 2: (storage.foldername(name))[1] = auth.uid()::text");
    addText("  Condition 3: (metadata->>'owner_id')::uuid = auth.uid()");
    addText("  Condition 4: (mimetype ~ 'image/.*')");
    y += 5;

    addText("3. Recommended SQL Editor commands:", 14, 'bold');
    addText("ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;", 9, 'normal', [80, 80, 80]);
    addText("CREATE POLICY \"Public View\" ON storage.objects FOR SELECT USING (true);", 9);
    addText("CREATE POLICY \"Folder Owner Management\" ON storage.objects FOR ALL TO authenticated ", 9);
    addText("USING ( bucket_id IN ('avatars', 'custom-media') ", 9);
    addText("  AND (storage.foldername(name))[1] = auth.uid()::text ", 9);
    addText("  AND (mimetype ~ 'image/.*') );", 9);
    y += 5;

    addText("--------------------------------------------------------------------------------", 10, 'normal', [150, 150, 150]);
    addText("Generated by CineMontauge Admin Portal", 8, 'italic', [150, 150, 150]);

    doc.save("CineMontauge_Supabase_Secure_Spec.pdf");
};
