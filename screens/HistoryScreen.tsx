    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {processedHistory.map(item => (
          <div key={item.logId} className="bg-bg-secondary/20 rounded-3xl border border-white/5 overflow-hidden flex flex-col shadow-xl animate-fade-in">
              <div className="w-full aspect-video relative cursor-pointer" onClick={() => onSelectShow(item.id, item.media_type)}>
                  <FallbackImage srcs={[getImageUrl(item.episodeStillPath || item.seasonPosterPath || item.poster_path, 'w780')]} placeholder={PLACEHOLDER_POSTER} alt={item.title} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                  <div className="absolute bottom-4 left-4 p-4 pl-8">
                      <h3 className="text-white font-black text-xl truncate">{item.title}</h3>
                      {item.media_type === 'tv' && <p className="text-xs font-black text-primary-accent uppercase tracking-widest">S{item.seasonNumber} {item.episodeNumber && `• E${item.episodeNumber}`}</p>}
                  </div>
              </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {searchHistory.map(item => (
          <div key={item.timestamp} className="relative group">
            <button onClick={() => onDelete(item.timestamp)} className="absolute -top-2 -right-2 z-30 p-1.5 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><XMarkIcon className="w-3.5 h-3.5" /></button>
            {(item as any).item ? (
              <div className="space-y-2">
                <ActionCard item={(item as any).item} onSelect={() => onSelectShow((item as any).item.id, (item as any).item.media_type)} onOpenAddToListModal={() => {}} onMarkShowAsWatched={() => {}} onToggleFavoriteShow={() => {}} isFavorite={false} isCompleted={false} showRatings={false} showSeriesInfo="hidden" />
